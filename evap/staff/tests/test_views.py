...
from evap.evaluation.models import Semester, UserProfile, Course, CourseType, TextAnswer, Contribution, \
                                   ...
                                   RatingAnswerCounter, TextanswerVisibility
...
class TestUserIndexView(WebTest):
    ...
    def test_num_queries_is_constant(self):
        ...
        course = mommy.make(Course, state="published", semester=semester, _participant_count=1, _voter_count=1)  # this triggers more checks in UserProfile.can_manager_delete
...
class TestSemesterAssignView(WebTest):
    ...
    def setUpTestData(cls):
        ...
        mommy.make(Contribution, contributor=mommy.make(UserProfile), course=course1,
                   responsible=True, can_edit=True, textanswer_visibility=TextanswerVisibility.GENERAL)
...
class TestSemesterTodoView(WebTestWith200Check):
    ...
    def test_todo(self):
        ...
        mommy.make(Contribution, course=course, contributor=user, responsible=True, can_edit=True, textanswer_visibility=TextanswerVisibility.GENERAL)
...
class TestSendReminderView(WebTest):
    ...
    def setUpTestData(cls):
        ...
        mommy.make(Contribution, course=course, contributor=responsible, responsible=True, can_edit=True, textanswer_visibility=TextanswerVisibility.GENERAL)
...
class TestSemesterParticipationDataExportView(WebTest):
    ...
    def setUpTestData(cls):
        ...
        mommy.make(Contribution, course=cls.course1, responsible=True, can_edit=True, textanswer_visibility=TextanswerVisibility.GENERAL)
        mommy.make(Contribution, course=cls.course2, responsible=True, can_edit=True, textanswer_visibility=TextanswerVisibility.GENERAL)
...
class TestCourseCreateView(WebTest):
    ...
    def test_course_create(self):
        ...
        form['contributions-0-textanswer_visibility'] = TextanswerVisibility.GENERAL
...
class TestCourseEditView(WebTest):
    ...
    def setUpTestData(cls):
        ...
        cls.contribution = mommy.make(Contribution, course=cls.course, contributor=responsible, responsible=True, can_edit=True, textanswer_visibility=TextanswerVisibility.GENERAL)
...
class TestSingleResultEditView(WebTestWith200Check):
    ...
    def setUpTestData(cls):
        ...
        contribution = mommy.make(Contribution, course=course, contributor=responsible, responsible=True, can_edit=True,
                                  textanswer_visibility=TextanswerVisibility.GENERAL, questionnaires=[Questionnaire.single_result_questionnaire()])
...
class TestCoursePreviewView(WebTestWith200Check):
...
class TestCourseTextAnswerView(WebTest):
    url = '/staff/semester/1/course/1/textanswers'
...
class TestCourseTextAnswerEditView(WebTest):
    url = '/staff/semester/1/course/1/textanswer/00000000-0000-0000-0000-000000000001/edit'
...
# Other Views
class TestCourseTextAnswersUpdatePublishView(WebTest):
    url = reverse("staff:course_textanswers_update_publish")
    csrf_checks = False
    ...
    def helper(self, old_state, expected_new_state, action, expect_errors=False):
        ...
        response = self.app.post(self.url, params={"id": textanswer.id, "action": action, "course_id": self.course.pk}, user="manager.user", expect_errors=expect_errors)
        if expect_errors:
...
class ParticipationArchivingTests(WebTest):
    ...
    def test_raise_403(self):
        ...
        self.app.get(semester_url + "course/create", user="manager", status=403)
...
class TestSemesterQuestionnaireAssignment(WebTest):
    ...
    def setUpTestData(cls):
        ...
        mommy.make(Contribution, contributor=cls.responsible, course=cls.course_1, responsible=True, can_edit=True, textanswer_visibility=TextanswerVisibility.GENERAL)
        mommy.make(Contribution, contributor=cls.responsible, course=cls.course_2, responsible=True, can_edit=True, textanswer_visibility=TextanswerVisibility.GENERAL)
